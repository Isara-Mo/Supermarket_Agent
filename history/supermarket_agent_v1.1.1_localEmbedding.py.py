...
def vector_store(text_chunks, db_name="faiss_db"):
    ...
    end_time = time.time()
    print(f"创建并保存向量数据库耗时: {end_time - start_time:.2f} 秒")
...
def process_product_csv(df):
    ...
    try:
        ...
        end_time = time.time()
        print(f"csv转文本耗时: {end_time - start_time:.2f} 秒")
        ...
        end_time = time.time()
        print(f"文本分割耗时: {end_time - start_time:.2f} 秒")

        # 创建向量数据库
        start_time = time.time()
        vector_store(final_chunks, "supermarket_db")
        end_time = time.time()
        print(f"创建向量数据库耗时: {end_time - start_time:.2f} 秒")
    ...
    with tab3:
        ...
        with col2:
            ...
            if product_csv:
                ...
                end_time = time.time()
                print(f"加载csv耗时: {end_time - start_time:.2f} 秒")
                ...
                if st.button("🚀 处理商品数据", use_container_width=True):
                    with st.spinner("📊 正在创建商品知识库..."):
                        try:
                            ...
                            end_time = time.time()
                            print(f"创建知识库数据耗时: {end_time - start_time:.2f} 秒")
                            if success:
