...
A RAG-based intelligent supermarket assistant that integrates "Supermarket Customer Service Retrieval & Recommendation" and "Data & Vector Database Management", supporting quick deployment and use.

### Key Features

1. **Intelligent Customer Service (RAG)**: Mounts product vector database, supports multi-turn conversations, answers questions about discounts, inventory, brands, locations, expiration dates, understands vague requirements and provides relevant recommendations. Supports CSV data import.
2. **Dynamic Model Adjustment Based on Question Complexity**: Leverages LangChain 1.0's updated intermediate block feature to dynamically adjust the Agent's model based on question complexity without rebuilding the Agent. This project uses a `bert_base_chinese` model to train a classification model that categorizes user input into three types:
...
3. **Configure Environment Variables** (Edit the `.env` file in the project root directory)
   
   ```env
   DASHSCOPE_API_KEY=your_dashscope_api_key
   ```
   
   - This project uses Alibaba DashScope's LLM and Embedding service.
...
- `app.py`: Main application entry point, UI interface
...
- v2.0: Refactored based on LangChain 1.0
  - Modularized functionality
  - Dynamic model adjustment based on question complexity
  - Optimized performance overhead from repeated Agent construction
...
1. **智能客服（RAG）**：挂载商品向量数据库，支持多轮对话，回答折扣、库存、品牌、位置、保质期等问题，理解模糊需求并给出相关推荐。支持 CSV 数据导入。
2. **根据问题复杂度动态调整 Agent 的模型**：利用 LangChain 1.0 更新后的中间块特性，可实现根据问题复杂度动态调整 Agent 中的模型，而无需重复构建 Agent。本项目中利用 `bert_base_chinese` 模型，训练了一个分类模型，根据用户的输入分为三类：
...
3. **配置环境变量**（编辑项目根目录下的 `.env`文件）
   
   ```env
   DASHSCOPE_API_KEY=你的dashscope密钥
   ```
   
   - 本项目使用阿里 DashScope 的 LLM 与 Embedding 服务。
...
- v2.0：基于 LangChain 1.0 重构
  - 功能模块化
  - 根据问题复杂度动态调整 Agent 的模型
  - 优化重复构建 Agent 的性能损耗
